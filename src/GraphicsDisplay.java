...
// Шрифт для подписей осей координат
        ...
        addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getButton() == MouseEvent.BUTTON3 && !graphicMemoryStack.isEmpty()) {
                    // Правый клик мыши: возвращаемся к предыдущему состоянию графика (отмена масштабирования)
                    ArrayList<Double[]> temp = graphicMemoryStack.pop(); // Вытаскиваем последний сохраненный масштаб
                    graphicsData = temp.toArray(graphicsData); // Восстанавливаем данные графика
            ...
            public void mousePressed(MouseEvent e) {
                ...
                if (button == MouseEvent.BUTTON1 && selectedPoint == null && graphicsIsOpen) {
                    // Левый клик мыши: начало выделения области для масштабирования
                    zoomPoint1 = new Point2D.Double(e.getX(), e.getY());
                    scalePoint1 = zoomPoint1; // Сохраняем начальную точку масштабирования
                    repaint();
                }

                if (button == MouseEvent.BUTTON1 && selectedPoint != null && graphicsIsOpen) {
                    // Левый клик мыши: начало перемещения выбранной точки
                    changeSelectedPoint = true; // Устанавливаем флаг перемещения
                    movedPoint = new Point2D.Double(e.getX(), e.getY()); // Сохраняем новую позицию точки
                    repaint();
                }
            }
            ...
            public void mouseReleased(MouseEvent e) {
                if (graphicsIsOpen) {
                    // Отпускание кнопки мыши: завершение масштабирования или перемещения точки
                    scalePoint2 = zoomPoint2; // Сохраняем конечную точку масштабирования
                    zoomPoint2 = null; // Сбрасываем временные данные
                    changeSelectedPoint = false; // Завершаем перемещение
                    movedPoint = null; // Сбрасываем выбранную точку
                    repaint(); // Обновляем график
                }
            }
            ...
            public void mouseEntered(MouseEvent e) {
                // Не реализовано: вызывается при входе курсора в компонент
            }

            @Override
            //мышь Вышла
            public void mouseExited(MouseEvent e) {
                // Не реализовано: вызывается при выходе курсора из компонента
            }
        });
        addMouseMotionListener(new MouseMotionListener() {
            ...
            public void mouseDragged(MouseEvent e) {
                if (graphicsIsOpen) {
                    if (zoomPoint1 != null) {
                        // Перетаскивание мыши для выделения области масштабирования
                        zoomPoint2 = new Point2D.Double(e.getX(), e.getY()); // Обновляем конечную точку выделения
                        repaint(); // Обновляем график с учетом выделения
                    }
                    if (changeSelectedPoint) {
                        // Перетаскивание выбранной точки
                        movedPoint.y = (new Point2D.Double(e.getX(), e.getY())).y; // Обновляем координаты точки
                        repaint(); // Обновляем график
                    }
                    // Проверяем, наведена ли мышь на одну из точек графика
                    for (Double[] point : graphicsData) {
                        Point2D.Double center = xyToPoint(point[0], point[1]);
                        if ((center.getX() - 5) < e.getX() && e.getX() < (center.getX() + 5) &&
                                (center.getY() - 5) < e.getY() && e.getY() < (center.getY() + 5)) {
                            selectedPoint = center; // Устанавливаем текущую выбранную точку
                            repaint();
                            break;
                        } else {
                            selectedPoint = null; // Сбрасываем выбор
                            repaint();
                        }
                    }
                }
            ...
            public void mouseMoved(MouseEvent e) {
                if (graphicsIsOpen) {
                    // Обновляем выбранную точку при движении мыши
                    for (Double[] point : graphicsData) {
                        Point2D.Double center = xyToPoint(point[0], point[1]);
                        if ((center.getX() - 5) < e.getX() && e.getX() < (center.getX() + 5) &&
                                (center.getY() - 5) < e.getY() && e.getY() < (center.getY() + 5)) {
...
// обрабатывать, так как мы файл создаѐм, а не открываем
    ...
    public void zoom() {
        // Проверяем, заданы ли обе точки для масштабирования
        if (scalePoint1 != null && scalePoint2 != null) {
            // Создаем временный список для хранения текущих данных графика
            ArrayList<Double[]> tempList = new ArrayList<>();

            // Проходим по всем точкам графика
            for (Double[] point : graphicsData) {
                // Преобразуем координаты текущей точки в систему координат графика
                Point2D.Double temp = xyToPoint(point[0], point[1]);

                // Сохраняем текущую точку в временный список
                tempList.add(new Double[]{point[0], point[1]});

                // Проверяем, находится ли точка внутри выделенной области масштабирования
                if (scalePoint1.getX() <= temp.getX() && scalePoint2.getX() >= temp.getX() &&
                        scalePoint1.getY() <= temp.getY() && scalePoint2.getY() >= temp.getY()) {
                    // Если точка попадает в выделенную область, добавляем её в новый список данных графика
                    graphicsDataList.add(new Double[]{point[0], point[1]});
                }
            }

            // Сохраняем текущее состояние графика в стек для возможности возврата
            graphicMemoryStack.add(tempList);

            // Перезаписываем данные графика только точками, находящимися в выделенной области
            graphicsData = new Double[graphicsDataList.size()][1];
            graphicsData = graphicsDataList.toArray(graphicsData);

            // Выводим значения точек нового масштаба в консоль
            for (Double[] point : graphicsData) {
                System.out.println("X: " + point[0] + ", Y: " + point[1]);
            }

            // Обновляем график для отображения масштабированных данных
            repaint();
        }
    }
